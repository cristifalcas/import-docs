...
SVN_LOCAL_BASE_PATH="$MY_DIR/svn_docs1"
...
#     $IC_PATH/sqlplus -S $ORA_USER/$ORA_PASS @$SQL_FILE $COL_SEP $SQL_OUTPUT_FILE
    ...
    SVN_URL_B="$SVN_BASE_URL"
    svn list --non-interactive --no-auth-cache --trust-server-cert --password "$SVN_PASS" --username "$SVN_USER" "$SVN_URL_B"  > "$MY_DIR/customers_ver"
# | tail -1  > "$MY_DIR/customers_ver"
# exit
#     while IFS=$COL_SEP read main; do
#     svn list --non-interactive --no-auth-cache --trust-server-cert --password "$SVN_PASS" --username "$SVN_USER" "$SVN_URL_B" | tail -1  > "$MY_DIR/customers_ver"
    while IFS="/" read main; do
echo zz $main "$MIND_MAX_VER"
	if [[ "$MIND_MAX_VER" > "$main" ]]; then
echo xx $main
	    main=$(echo $main)
...
exit
    ...
    svn list --non-interactive --no-auth-cache --trust-server-cert --password "$SVN_PASS" --username "$SVN_USER" "$SVN_URL" > "$MY_DIR/customers"
...
#     done <$SQL_OUTPUT_FILE
	...
	svn list --non-interactive --no-auth-cache --trust-server-cert --password "$SVN_PASS" --username "$SVN_USER" "$SVN_URL" | tail -1  > "$MY_DIR/customers_ver"
	while IFS="/" read ver; do
	    SVN_URL="$SVN_URL/$ver"
