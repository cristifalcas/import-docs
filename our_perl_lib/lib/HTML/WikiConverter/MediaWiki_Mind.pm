package HTML::WikiConverter::MediaWiki_Mind;
...
my @common_attrs = qw/ id class title style /;
...
# );
my @tablealign_attrs = qw/ char charoff /;
my @tablecell_attrs = qw(
  abbr axis headers scope rowspan
  colspan nowrap bgcolor
);
...
#     strong => { start => "'''", end => "'''", line_format => 'single' },
    ...
    ul => { line_format => 'multi', block => 1, preserve => 1 },
    ol => { line_format => 'multi', block => 1, preserve => 1, attributes => [ @common_attrs, qw/ start type / ] },
    dl => { line_format => 'multi', block => 1 },

    li => { preserve => 1 },
    dt => { start => \&_li_start, trim => 'leading' },
    dd => { alias => 'dt' },
...
#   return sprintf '[[Image:%s|%s]]', $img, $alt if $alt;
  return sprintf '[[Image:%s|%spx]]', $img, $width if $width;
...
sub _table_start {
  ...
  my @table_attrs = (
    @common_attrs,
    qw/ summary border cellspacing
        cellpadding bgcolor frame rules /
  );
