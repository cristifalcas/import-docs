...
class LuceneResult extends SearchResult {
	...
	function LuceneResult( $lines, $method ) {
		...
		if (($namespace>=100 && $namespace<=200) || 
			$namespace==500 || $namespace==600 || $namespace==700){
			$res = $dbw->query( "select substring_index(a.old_text,'\n',1) text from 
				...
				and c.page_title='$title'
				and c.page_namespace='$namespace';" );
			while( $row = $dbw->fetchObject( $res ) ) {
			    $cur[] = $row->text;
			    $appending = $cur[0];// Provides: <body text='black'>
			    ...
			    $this->mHighlightText="<u><b>$appending</b></u></br>".$this->mHighlightText;
			mysql_free_result($res);
		}
		wfDebug( "Cristi line: '$namespace.$title.$this->mHighlightText'\n" );
		//$snippet = "<b>coco</b></br>".$snippet;
